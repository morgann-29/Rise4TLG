...
function ProjectPeriods() {
  ...
  useEffect(() => {
    loadData()
  }, [showDeleted, groupId, projectId])

  const loadData = async () => {
    try {
      setLoading(true)
      if (isCoachContext) {
        const [groupData, projectData, periodsData] = await Promise.all([
          coachService.getGroupBasic(groupId),
          coachService.getProjectDetail(groupId, projectId),
          coachService.getProjectPeriods(groupId, projectId, showDeleted)
        ])
        setGroup(groupData)
        setProject(projectData)
        setPeriods(periodsData || [])
      } else {
        const [projectsData, periodsData] = await Promise.all([
          navigantService.getMyProjects(),
          navigantService.getPeriods(projectId, showDeleted)
        ])
        const projectData = projectsData.find(p => p.id === projectId)
        setProject(projectData || { id: projectId, name: 'Mon Projet' })
        setPeriods(periodsData || [])
      }
      setError(null)
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (isCoachContext) {
        if (editingItem) {
          await coachService.updateProjectPeriod(groupId, projectId, editingItem.id, submitData)
        } else {
          await coachService.createProjectPeriod(groupId, projectId, submitData)
        }
      } else {
        if (editingItem) {
          await navigantService.updatePeriod(projectId, editingItem.id, submitData)
        } else {
          await navigantService.createPeriod(projectId, submitData)
        }
      }
      setShowModal(false)
      setEditingItem(null)
  ...
  const handleDelete = async (item) => {
    ...
    try {
      if (isCoachContext) {
        await coachService.deleteProjectPeriod(groupId, projectId, item.id)
      } else {
        await navigantService.deletePeriod(projectId, item.id)
      }
      await loadData()
    } catch (err) {
  ...
  const handleRestore = async (item) => {
    try {
      if (isCoachContext) {
        await coachService.restoreProjectPeriod(groupId, projectId, item.id)
      } else {
        await navigantService.restorePeriod(projectId, item.id)
      }
      await loadData()
    } catch (err) {
  ...
  const openEditModal = (item, e) => {
    e.stopPropagation()
    // Cannot edit name/dates if has master
    if (item.period_master_id) {
      alert('Cette periode est liee a une periode de groupe. Seul le contenu peut etre modifie depuis la page detail.')
      return
    }
    setEditingItem(item)
    setFormData({
  ...
  const getDetailUrl = (periodId) => {
    if (isCoachContext) {
      return `/coach/groups/${groupId}/projects/${projectId}/periods/${periodId}`
    }
    return `/navigant/projects/${projectId}/periods/${periodId}`
  }
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            {isCoachContext && group && (
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">
                {group.name} / {project?.name}
              </div>
            )}
            {!isCoachContext && project && (
              <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">
                {project.name}
              </div>
            )}
          ...
          {periods.length === 0 ? (
            ...
            <ul className="divide-y divide-gray-200 dark:divide-gray-700">
              {periods.map((item) => (
                <li
                  key={item.id}
                  onClick={() => navigate(getDetailUrl(item.id))}
                  ...
                  <div className="flex items-center justify-between">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-1 flex items-center space-x-4 text-sm text-gray-500 dark:text-gray-400">
                        ...
                        <span>
                          {item.session_count} seance{item.session_count > 1 ? 's' : ''}
                        </span>
                      </div>
                    </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Ex: Semaine de preparation"
                  />
                </div>
