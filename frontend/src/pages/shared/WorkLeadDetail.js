...
// Status labels and colors (coherent with GroupWorkLeadDetail)
const STATUS_CONFIG = {
...
function WorkLeadDetail() {
  ...
  const getBackUrl = useCallback(() => {
    if (isCoach && groupId) {
      return `/coach/groups/${groupId}/projects/${projectId}/work-leads`
    }
    return `/navigant/projects/${projectId}/work-leads`
  }, [isCoach, groupId, projectId])
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      let workLeadData, projectData

      if (isCoach && groupId) {
        // Coach context
        [workLeadData, projectData] = await Promise.all([
          coachService.getProjectWorkLead(groupId, projectId, workLeadId),
          coachService.getProjectDetail(groupId, projectId)
        ])
      } else {
        // Navigant context
        [workLeadData, projectData] = await Promise.all([
          navigantService.getWorkLead(projectId, workLeadId),
  ...
  }, [isCoach, groupId, projectId, workLeadId])
  ...
  const loadSessionHistory = useCallback(async (offset = 0, append = false) => {
    try {
      ...
      let data
      if (isCoach && groupId) {
        data = await coachService.getProjectWorkLeadSessions(groupId, projectId, workLeadId, offset, SESSIONS_PER_PAGE)
      } else {
        data = await navigantService.getWorkLeadSessions(projectId, workLeadId, offset, SESSIONS_PER_PAGE)
      }
  ...
  const handleSaveContent = async (content) => {
    if (isCoach && groupId) {
      await coachService.updateProjectWorkLead(groupId, projectId, workLeadId, {
        name: workLead.name,
        work_lead_type_id: workLead.work_lead_type_id,
        content
      })
    } else {
      await navigantService.updateWorkLead(projectId, workLeadId, {
        name: workLead.name,
        work_lead_type_id: workLead.work_lead_type_id,
        content
      })
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="flex items-start justify-between">
          {/* Left side: back button, project name, title, type badge */}
