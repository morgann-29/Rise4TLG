...
function GroupSessions() {
  ...
  const handleDelete = async (item) => {
    ...
    try {
      await coachService.deleteGroupSession(groupId, item.id)
      await loadData()
    } catch (err) {
  ...
  const handleRowClick = (item) => {
    if (!item.is_deleted) {
      navigate(`/coach/groups/${groupId}/sessions/${item.id}`)
    }
  }
        ...
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
              Seances effectuees pour ce groupe
            </p>
          </div>
              ...
              {sessions.length === 0 && (
                <tr>
                  <td colSpan="5" className="px-6 py-12 text-center text-gray-500 dark:text-gray-400">
                    ...
                    <p>Aucune seance</p>
                    <p className="text-sm mt-1">Creez votre premiere seance pour ce groupe</p>
                  </td>
                </tr>
              )}
            </tbody>
