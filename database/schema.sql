...
CREATE TRIGGER update_type_profile_updated_at BEFORE
UPDATE ON type_profile FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- Données initiales type_profile
INSERT INTO type_profile (name)
VALUES ('Admin'),
    ('Super Coach'),
    ('Coach'),
    ('Navigant') ON CONFLICT (name) DO NOTHING;
...
CREATE TRIGGER update_type_support_updated_at BEFORE
UPDATE ON type_support FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_type_seance_updated_at BEFORE
UPDATE ON type_seance FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- ============================================
...
CREATE TRIGGER update_project_updated_at BEFORE
UPDATE ON project FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- Trigger: Vérifier que profile_id est un Navigant (type_profile_id = 4)
CREATE OR REPLACE FUNCTION check_project_owner_is_navigant() RETURNS TRIGGER AS $$ BEGIN IF NOT EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.id = NEW.profile_id
            AND p.type_profile_id = 4 -- Navigant
    ) THEN RAISE EXCEPTION 'Le propriétaire du projet doit être un Navigant (type_profile_id = 4)';
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_check_project_owner_is_navigant BEFORE
INSERT
    OR
UPDATE OF profile_id ON project FOR EACH ROW EXECUTE FUNCTION check_project_owner_is_navigant();
-- Groupe
CREATE TABLE IF NOT EXISTS "group" (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name TEXT NOT NULL,
    type_support_id INTEGER REFERENCES type_support(id) ON DELETE
    SET NULL,
        is_deleted BOOLEAN DEFAULT FALSE,
        created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
        updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
...
CREATE INDEX idx_group_is_deleted ON "group"(is_deleted)
WHERE is_deleted = FALSE;
CREATE TRIGGER update_group_updated_at BEFORE
UPDATE ON "group" FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
-- NE PAS UTILISER, sert probablement a rien, n'a pas été supprimé au cas où mais le sera probalement 
...
CREATE TABLE IF NOT EXISTS session_master (
    ...
    -- créateur (nullable pour templates)
    group_id UUID REFERENCES "group"(id) ON DELETE RESTRICT,
    -- nullable pour sessions hors groupe ou templates
    type_seance_id INTEGER NOT NULL REFERENCES type_seance(id) ON DELETE RESTRICT,
    coach_id UUID REFERENCES profile(id) ON DELETE
    SET NULL,
        -- coach présent (peut être différent du créateur)
        date_start TIMESTAMP WITH TIME ZONE,
        -- nullable pour séances "template"
...
CREATE INDEX idx_session_master_is_deleted ON session_master(is_deleted)
WHERE is_deleted = FALSE;
CREATE TRIGGER update_session_master_updated_at BEFORE
UPDATE ON session_master FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- Trigger: Vérifier que coach_id est un Coach (type_profile_id = 3)
CREATE OR REPLACE FUNCTION check_session_master_coach_is_coach() RETURNS TRIGGER AS $$ BEGIN IF NEW.coach_id IS NOT NULL
    AND NOT EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.id = NEW.coach_id
            AND p.type_profile_id = 3 -- Coach
    ) THEN RAISE EXCEPTION 'Le coach de la session doit être un Coach (type_profile_id = 3)';
END IF;
RETURN NEW;
END;
$$ LANGUAGE plpgsql;
CREATE TRIGGER trigger_check_session_master_coach BEFORE
INSERT
    OR
UPDATE OF coach_id ON session_master FOR EACH ROW EXECUTE FUNCTION check_session_master_coach_is_coach();
-- Session (session individuelle, liée à un projet)
CREATE TABLE IF NOT EXISTS session (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    name TEXT NOT NULL,
    project_id UUID NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    session_master_id UUID REFERENCES session_master(id) ON DELETE
    SET NULL,
        type_seance_id INTEGER NOT NULL REFERENCES type_seance(id) ON DELETE RESTRICT,
        date_start TIMESTAMP WITH TIME ZONE,
        -- nullable pour séances "template"
...
CREATE INDEX idx_session_is_deleted ON session(is_deleted)
WHERE is_deleted = FALSE;
CREATE TRIGGER update_session_updated_at BEFORE
UPDATE ON session FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
UPDATE ON work_lead_type FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE INDEX idx_work_lead_master_is_deleted ON work_lead_master(is_deleted)
WHERE is_deleted = FALSE;
CREATE TRIGGER update_work_lead_master_updated_at BEFORE
UPDATE ON work_lead_master FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- Work Lead (axe de travail au niveau projet)
CREATE TABLE IF NOT EXISTS work_lead (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    project_id UUID NOT NULL REFERENCES project(id) ON DELETE CASCADE,
    work_lead_master_id UUID REFERENCES work_lead_master(id) ON DELETE
    SET NULL,
        work_lead_type_id UUID NOT NULL REFERENCES work_lead_type(id) ON DELETE RESTRICT,
        name TEXT NOT NULL,
        content TEXT,
...
CREATE INDEX idx_work_lead_is_deleted ON work_lead(is_deleted)
WHERE is_deleted = FALSE;
CREATE TRIGGER update_work_lead_updated_at BEFORE
UPDATE ON work_lead FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TABLE IF NOT EXISTS session_work_lead (
    ...
    override_master BOOLEAN DEFAULT NULL,
    -- NULL=créé directement, FALSE=synchro master, TRUE=personnalisé
    profile_id UUID REFERENCES profile(id) ON DELETE
    SET NULL,
        -- audit: qui a cree/modifie
        created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
        updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
CREATE TRIGGER update_session_work_lead_updated_at BEFORE
UPDATE ON session_work_lead FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_session_master_work_lead_master_updated_at BEFORE
UPDATE ON session_master_work_lead_master FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
-- ============================================
-- PERIODS (Periodes d'entrainement)
-- ============================================
-- Period Master (periode de groupe, creee par un Coach)
...
CREATE TABLE IF NOT EXISTS files (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    origin_entity_type TEXT NOT NULL CHECK (
...
CREATE TABLE IF NOT EXISTS weather_data (
    ...
    wind_direction_degrees INTEGER CHECK (
...
-- --------------------------------------------
CREATE POLICY "Authenticated read type_profile" ON type_profile FOR
SELECT TO authenticated USING (true);
CREATE POLICY "Authenticated read type_support" ON type_support FOR
SELECT TO authenticated USING (true);
CREATE POLICY "Authenticated read type_seance" ON type_seance FOR
SELECT TO authenticated USING (is_deleted = FALSE);
-- --------------------------------------------
-- Profile: chacun voit le sien, admin voit tout
-- --------------------------------------------
CREATE POLICY "Users read own profile" ON profile FOR
SELECT TO authenticated USING (user_uid = auth.uid());
CREATE POLICY "Admin full access profile" ON profile FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
...
-- --------------------------------------------
-- Admin: accès total
CREATE POLICY "Admin full access project" ON project FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.user_uid = auth.uid()
...
CREATE POLICY "Super Coach read all projects" ON project FOR
...
CREATE POLICY "Coach access group projects" ON project FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
...
CREATE POLICY "Navigant access own project" ON project FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.user_uid = auth.uid()
...
-- Group
-- --------------------------------------------
CREATE POLICY "Admin full access group" ON "group" FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.user_uid = auth.uid()
...
CREATE POLICY "Super Coach read all groups" ON "group" FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
CREATE POLICY "Coach access own groups" ON "group" FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
-- --------------------------------------------
CREATE POLICY "Admin full access project_session_master" ON project_session_master FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.user_uid = auth.uid()
...
CREATE POLICY "Super Coach read all project_session_master" ON project_session_master FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
CREATE POLICY "Coach access group project_session_master" ON project_session_master FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
            JOIN group_profile gp ON gp.profile_id = p.id
...
CREATE POLICY "Navigant read own project_session_master" ON project_session_master FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
-- Session Master
-- --------------------------------------------
CREATE POLICY "Admin full access session_master" ON session_master FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.user_uid = auth.uid()
...
CREATE POLICY "Super Coach read all session_master" ON session_master FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
CREATE POLICY "Coach access group session_master" ON session_master FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
            JOIN group_profile gp ON gp.profile_id = p.id
...
CREATE POLICY "Navigant read linked session_master" ON session_master FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
-- Session
-- --------------------------------------------
CREATE POLICY "Admin full access session" ON session FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.user_uid = auth.uid()
...
CREATE POLICY "Super Coach read all sessions" ON session FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
CREATE POLICY "Coach access group sessions" ON session FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
            JOIN group_profile gp ON gp.profile_id = p.id
...
CREATE POLICY "Navigant access own sessions" ON session FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
            JOIN project proj ON proj.profile_id = p.id
...
-- Weather Data
-- --------------------------------------------
CREATE POLICY "Admin full access weather_data" ON weather_data FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
        WHERE p.user_uid = auth.uid()
...
CREATE POLICY "Super Coach read all weather_data" ON weather_data FOR
SELECT TO authenticated USING (
        EXISTS (
            SELECT 1
            FROM profile p
...
CREATE POLICY "Coach access weather_data" ON weather_data FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
            JOIN group_profile gp ON gp.profile_id = p.id
        WHERE p.user_uid = auth.uid()
            ...
            AND (
                ...
                (
                    weather_data.entity_type = 'session_master'
                    AND EXISTS (
                        SELECT 1
                ...
                OR -- Session d'un projet de son groupe
                (
                    weather_data.entity_type = 'session'
                    AND EXISTS (
                        SELECT 1
                        FROM session s
...
CREATE POLICY "Navigant access weather_data" ON weather_data FOR ALL TO authenticated USING (
    EXISTS (
        SELECT 1
        FROM profile p
            JOIN project proj ON proj.profile_id = p.id
        WHERE p.user_uid = auth.uid()
            ...
            AND (
                ...
                OR -- Session master liée à une de ses sessions
                (
                    weather_data.entity_type = 'session_master'
                    AND EXISTS (
                        SELECT 1
